...
export class BattleMap extends Immutable<BattleMap> {
  static EMPTY = new BattleMap(Common.create('', ImmutableType.map), '', [], 100, [], [], 'pink', 0, 0, {
    name: '',
    url: '',
  });
  ...
  static create(name: string, bases: string[] = []): BattleMap {
    return new BattleMap(Common.create(name, ImmutableType.map), '', [], 0, [], [], '', 0, 0, {
      name: '',
      url: '',
    });
  }
  ...
  static fromProto(proto: MapsProto.Map, productContent: ProductContent): BattleMap {
    return new BattleMap(
      Common.fromProto(proto.getCommon(), productContent, ImmutableType.map, true),
      productContent.name,
