...
import { MatLegacyDialog as MatDialog, MatLegacyDialogRef as MatDialogRef, MatLegacyDialogState as MatDialogState, MAT_LEGACY_DIALOG_DATA as MAT_DIALOG_DATA } from '@angular/material/legacy-dialog';
...
export class LocationEditDialogComponent {
  ...
  onAddFilter(index: number) {
    this.location = this.location.withFilters([...this.location.filters.slice(0, index + 1), 
      EMPTY,
      ...this.location.filters.slice(index + 1)]);
  }

  onDeleteFilter(index: number) {
    this.location = this.location.withFilters([...this.location.filters.slice(0, index), 
      ...this.location.filters.slice(index + 1)]);
  }
