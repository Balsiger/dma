const YEAR_NAMES: Map<number, string> = new Map([
...
class Day {
  ...
  matches(day: number, leap = false): boolean {
    return this.start + (leap ? this.leap : 0) <= day && this.end + (leap ? this.leap : 0) >= day;
  }
  ...
  formatDay(day: number, leap = false): string {
    return `${day + 1 - this.start - (leap ? this.leap : 0)}`;
  }
  ...
  static createMonth(name: string, synonyms: string[], similar: string, leap = 0): Day {
    Day.count += 30;
    return new Day(Day.count - 30, Day.count - 1, leap, name, synonyms, similar);
  }

  static createDay(name: string, leap = 0): Day {
    return new Day(Day.count, Day.count++, leap, name, [], '');
  }
}

const DAY_NAMES: Day[] = [
  new Day(-1, -1, 214, 'Shieldmeet', [], ''),
  Day.createMonth('Hammer', ['Deepwinter'], 'January'),
  Day.createDay('Midwinter'),
  Day.createMonth('Alturiak', ['The Claw of Winter', 'The Claws of the Cold'], 'February'),
  Day.createMonth('Ches', ['The Claw of Sunsets'], 'March'),
  Day.createMonth('Tarsakh', ['THe Claw of Storms'], 'April'),
  Day.createDay('Greengrass'),
  Day.createMonth('Mirtul', ['The Melting'], 'May'),
  Day.createMonth('Kythorn', ['The Time of Flowers'], 'June'),
  Day.createMonth('Flamerule', ['Summertide'], 'July'),
  Day.createDay('Midsummer'),
...
export class Dates {
  static getYearName(year: number) {
    return YEAR_NAMES.get(year) || 'Unknown Year';
  }

  static getDayName(day: number, leap = false): Day {
    for (const name of DAY_NAMES) {
      if (name.matches(day, leap)) {
        return name;
      }
    }
  ...
  static formatDate(day: number, leap = false): string {
    return Dates.getDayName(day, leap).format(day, leap);
  }

  static formatDay(day: number, leap = false): string {
    return Dates.getDayName(day, leap).formatDay(day, leap);
  }

  static formatMonth(day: number, leap = false): string {
    return Dates.getDayName(day, leap).formatMonth();
  }
}
