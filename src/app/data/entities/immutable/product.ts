...
export class Product extends Immutable<Product> {
  ...
  override matches(selections: Map<string, any>): boolean {
    for (const [label, value] of selections.entries()) {
      ...
      if (label === 'System' && !Immutable.includes(this.system, value)) {
        return false;
      }
    }
  ...
  static fromProto(proto: ProductProto, productContent: ProductContent): Product {
    return new Product(
      Common.fromProto(proto.getCommon(), productContent, ImmutableType.product),
      productContent.name,
      proto.getTitle(),
      proto.getLeader(),
      proto.getSubtitle(),
  ...
  static create(name: string, bases: string[] = []): Product {
    return new Product(
      Common.create(name, ImmutableType.product),
      '',
      '(unknown)',
      '',
      '',
